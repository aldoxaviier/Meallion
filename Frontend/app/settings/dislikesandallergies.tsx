...
const Allergies = () => {
    ...
    useEffect(() => {
        setSelectedIngredients(profileDataContext?.profileData?.allergies || []);
    }, []);
    ...
    const next = async () => {
        ...
        setIsLoading(true);
        try {
        const response: any = await api.put('/profile/updateProfile', { allergies: selectedIngredients });
    ...
    console.log("ProfileDataContext allergies:", profileDataContext?.profileData);

    return (
        <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
        <GestureHandlerRootView style={{ flex: 1 }}>
        <SafeAreaView className="bg-secondary-400 flex-1" >
            <View className="h-full w-full flex flex-col gap-4 px-6">
            ...
            <View className="flex flex-col gap-44 flex-1">
                ...
                >
                {isLoading ? (
                    <ActivityIndicator color="#F4EEE5" />
                ) : (
