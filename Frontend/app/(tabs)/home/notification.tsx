...
export default function Notification() {
  ...
  const notifStyles = {
    meal_reminder: { icon: "alarm" as const, color: "#0d9488", bg: "bg-teal-100" },
    follow: { icon: "person-add" as const, color: "#6366f1", bg: "bg-indigo-100" },
    review: { icon: "star" as const, color: "#eab308", bg: "bg-yellow-100" },
  } as const;
  ...
  return (
    <View className="flex-1 bg-[#f0ead8]">
      ...
      <SectionList
        ...
        renderSectionHeader={({ section }) => (
          <View className="items-center my-3">
            <View className="bg-[#e8e2d5] rounded-full px-4 py-1">
              <Text className="text-xs font-brsegma-600 text-gray-600">{section.title}</Text>
            </View>
          </View>
        )}
        renderItem={({ item }) => {
          ...
          return (
            <TouchableOpacity
              ...
              activeOpacity={0.75}
              className={`mx-4 mb-2 flex-row items-center rounded-2xl bg-white px-3 py-3 `}
            ...
            >
              ...
              <View className="flex-1">
                <Text className={`text-sm font-brsegma-700 ${item.is_checked ? "text-black" : "text-gray-800"}`} numberOfLines={1}>
                  {item.title}
                </Text>
                <Text className={`text-xs mt-0.5 leading-4 ${item.is_checked ? "text-black" : "text-gray-500"}`} numberOfLines={2}>
                  {item.content}
                </Text>
              </View>
        ...
        ListEmptyComponent={
          <View className="items-center justify-center mt-32">
            ...
            <Text className="text-gray-500 font-brsegma-500 text-sm mt-3">No notifications yet.</Text>
          </View>
        }
      />
    </View>
